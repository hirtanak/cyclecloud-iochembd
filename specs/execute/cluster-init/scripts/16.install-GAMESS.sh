...
if [[ ${GAMESS_BUILD} = "gamess-sockets" ]]; then
   if [[ ! -f ${GAMESS_BUILD}/gamess.00.x ]]; then
      ...
      /usr/bin/csh /mnt/cluster-init/ioChem-BD/execute/files/expect-sockets
...
if [[ ${GAMESS_BUILD} = "gamess-impi" ]]; then
   if [[ ! -f ${GAMESS_BUILD}/gamess.00.x ]]; then
      ...
      /usr/bin/csh /mnt/cluster-init/ioChem-BD/execute/files/expect-impi
      # buils settings
      declare I_MPI_ROOT
      declare CLASSPATH
...
cp /opt/cycle/jetpack/logs/cluster-init/ioChem-BD/execute/scripts/16.install-${SW}.sh.out ${HOMEDIR}/logs/
